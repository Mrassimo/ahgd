...
import polars as pl
from src.pipelines.core_etl_pipeline import CoreETLPipeline, run_sa1_etl_pipeline
...
def create_demo_health_data():
    ...
    data = pl.DataFrame({
        'health_indicator': ['diabetes_rate', 'obesity_rate', 'smoking_rate', 'mental_health_score', 'life_expectancy'],
        'postcode': ['2000', '3000', '4000', '5000', '6000'],  # Mixed postcodes
        'sa2_code': ['101021007', '202032008', '305045009', '401028005', '501013001'],  # SA2 codes
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            for stage, result in results['stage_results'].items():
                ...
                print(f"   {status_emoji} {stage.upper()}: {result['status']} ({result['records_processed']} records)")
            ...
            if output_path.exists():
                ...
                sa1_columns = [col for col in processed_data.columns if 'sa1' in col.lower() or col in ['processing_method', 'processing_status']]
                if sa1_columns:
