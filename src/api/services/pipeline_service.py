...
from ...utils.interfaces import (
    AHGDException, PipelineError, ProcessingStatus,
    ProcessingMetadata, AuditTrail
)
from ..models.common import PipelineRun, PipelineStageResult, StatusEnum, PipelineStage
from ..models.requests import PipelineRunRequest, PaginationRequest
from ..models.responses import PipelineRunResponse, StatusResponse, BulkOperationResponse
from ..exceptions import (
    ServiceUnavailableException, PipelineException, 
    raise_pipeline_error, ResourceNotFoundException
)
...
class PipelineService:
    ...
    async def execute_pipeline(
        ...
        try:
            ...
            if not request.force_rerun:
                recent_run = await self._check_recent_successful_run(
                    request.pipeline_name
                )
                if recent_run:
                    logger.info(
        ...
        try:
            ...
            if run_id in self._active_runs:
                ...
                return StatusResponse(
                    ...
                    result_url=f"/api/v1/pipelines/{run_id}" if pipeline_run.status == StatusEnum.COMPLETED else None,
            ...
            if historical_run:
                return StatusResponse(
                    ...
                    current_step="Completed" if historical_run["status"] == "completed" else "Failed",
                    estimated_completion=None,
                    result_url=f"/api/v1/pipelines/{run_id}" if historical_run["status"] == "completed" else None,
    ...
    async def list_pipeline_runs(
        ...
        try:
            ...
            if pipeline_name_filter:
                filtered_runs = [run for run in filtered_runs if run["pipeline_name"] == pipeline_name_filter]
            ...
            raise ServiceUnavailableException(
                "pipeline_service",
                f"Pipeline listing failed: {str(e)}"
            )
    ...
    async def cancel_pipeline_run(
        ...
        try:
            ...
            raise ServiceUnavailableException(
                "pipeline_service",
                f"Pipeline cancellation failed: {str(e)}"
            )
    ...
    async def _check_recent_successful_run(
        ...
        for run_id, run_info in self._active_runs.items():
            pipeline_run = run_info["pipeline_run"]
            if (pipeline_run.pipeline_name == pipeline_name and 
                pipeline_run.status == StatusEnum.COMPLETED and
                pipeline_run.start_time >= cutoff_time):
                return {"run_id": run_id, "start_time": pipeline_run.start_time}
        
        # Check historical runs
        for run in self._run_history:
            if (run["pipeline_name"] == pipeline_name and
                run["status"] == "completed" and 
                run["start_time"] >= cutoff_time):
                return run
    ...
    async def _check_concurrency_limits(
        ...
        if not pipeline_config.get("max_parallel", True):
            ...
            for run_info in self._active_runs.values():
                if (run_info["pipeline_run"].pipeline_name == pipeline_name and
                    run_info["pipeline_run"].status == StatusEnum.IN_PROGRESS):
                    raise PipelineException(
    ...
    async def _execute_pipeline_async(
        ...
        try:
            ...
            if pipeline_run.failed_stages > 0:
                ...
                pipeline_run.error_message = pipeline_run.error_message or "One or more stages failed"
            else:
    ...
    async def _execute_pipeline_stage(
        ...
        try:
            ...
            stage_result.performance_metrics = {
                "records_per_second": processed_records / max(1, stage_result.duration_seconds or 1),
