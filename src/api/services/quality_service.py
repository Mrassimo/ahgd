...
from ...utils.interfaces import (
    AHGDException, DataQualityError, ValidationError,
    DataRecord, DataBatch, MetadataDict
)
from ..models.common import QualityScore, GeographicLevel, SA1Code
from ..models.requests import QualityMetricsRequest, QualityAnalysisRequest, GeographicQuery
from ..models.responses import (
    QualityMetricsResponse, QualityAnalysisResponse, GeographicAnalysisResponse
)
from ..exceptions import ServiceUnavailableException, raise_validation_error
...
class QualityMetricsService:
    ...
    async def _perform_geographic_analysis(
        self, 
        geographic_query: GeographicQuery
    ...
    async def _generate_visualisation_data(
        ...
        dimensional_analysis: Dict[str, Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    async def _generate_prioritised_recommendations(
        ...
        recommendations.sort(
            key=lambda x: (priority_order.get(x["priority"], 0), x["current_score"]),
            reverse=True
        )
