...
class ClimateRecord(GeographicModel, DataQualityMixin, TimestampedModel):
    ...
    solar_radiation_mj: Optional[confloat(ge=0)] = Field(
        None, description="Solar radiation (MJ/m²)"
    )
    ...
    frost_day: Optional[bool] = Field(None, description="Whether minimum temperature below 2°C")
...
class AirQualityRecord(GeographicModel, DataQualityMixin, TimestampedModel):
    ...
    pm2_5_ugm3: Optional[confloat(ge=0)] = Field(None, description="PM2.5 concentration (μg/m³)")

    pm10_ugm3: Optional[confloat(ge=0)] = Field(None, description="PM10 concentration (μg/m³)")

    ozone_ugm3: Optional[confloat(ge=0)] = Field(None, description="Ozone concentration (μg/m³)")

    no2_ugm3: Optional[confloat(ge=0)] = Field(
        None, description="Nitrogen dioxide concentration (μg/m³)"
    )

    so2_ugm3: Optional[confloat(ge=0)] = Field(
        None, description="Sulfur dioxide concentration (μg/m³)"
    )

    co_mgm3: Optional[confloat(ge=0)] = Field(
        None, description="Carbon monoxide concentration (mg/m³)"
    )
