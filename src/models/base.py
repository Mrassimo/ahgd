...
class TimestampedModel(BaseModel):
    ...
    reference_date: Optional[date] = Field(
        None,
        description="Date this data record represents (e.g., census collection date)"
    )
    
    # Data processing timestamps
    extracted_at: Optional[datetime] = Field(
        None,
        description="When this record was extracted from source"
    )
    
    processed_at: Optional[datetime] = Field(
        None, 
        description="When this record was processed and validated"
    )
    
    # Data version tracking
    source_version: Optional[str] = Field(
        None,
        description="Version identifier of the source data"
    )
...
class GeographicModel(TimestampedModel):
    ...
    state_code: constr(pattern=r"^[1-8]$", min_length=1, max_length=1) = Field(
        ...,
        description="ABS state/territory code (1-8)",
        examples=["1", "2", "3"]
    )
    ...
    def validate_state_name(cls, v):
        ...
        state_mapping = {
            ...
            # Alternative forms
            "Other Territories": "OT", "OT": "OT"
        }
        
        standardised = state_mapping.get(v.strip())
        if not standardised:
            raise ValueError(f"Invalid state name: {v}. Must be one of {list(state_mapping.keys())}")
        return standardised
...
class DataQualityMixin(BaseModel):
    ...
    has_missing_data: bool = Field(
        False,
        description="Whether this record has missing required fields"
    )
    ...
    quality_score: Optional[float] = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Data quality score from 0.0 (poor) to 1.0 (excellent)"
    )
    
    validation_errors: Optional[list[str]] = Field(
        None,
        description="List of validation warnings or non-fatal errors"
    )
...
class PopulationMixin(BaseModel):
    ...
    population_total: Optional[int] = Field(
        None,
        ge=0,
        description="Total population count"
    )
    
    population_male: Optional[int] = Field(
        None, 
        ge=0,
        description="Male population count"
    )
    
    population_female: Optional[int] = Field(
        None,
        ge=0,
    ...
    def validate_gender_population_sum(cls, v, values):
        ...
        if v is not None and 'population_total' in values:
            ...
            if total is not None and v > total:
                raise ValueError(f"Gender population ({v}) cannot exceed total population ({total})")
        return v