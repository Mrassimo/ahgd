...
def main():
    ...
    seifa_min, seifa_max = float(df_pandas['seifa_score'].min()), float(df_pandas['seifa_score'].max())
    seifa_range = st.sidebar.slider("SEIFA Score (Socioeconomic):", seifa_min, seifa_max, (seifa_min, seifa_max))
    ...
    with col2:
        ...
        st.metric(
            "Total Population", 
            f"{total_pop:,}",
            f"Across {len(selected_states)} states"
        )
    ...
    with col4:
        ...
        st.metric(
            "Avg GPs per 1,000", 
            f"{avg_access:.2f}",
            f"National target: 1.0+"
        )
    
    # Main content tabs
    tab1, tab2, tab3, tab4 = st.tabs(["📊 Health Overview", "🗺️ Geographic Analysis", "📈 Health Trends", "🔍 Data Explorer"])
    ...
    with tab1:
        ...
        with col1:
            ...
            state_diabetes = filtered_df.groupby('state')['diabetes_prevalence'].agg(['mean', 'std']).reset_index()
            ...
            fig1.add_hline(y=5.1, line_dash="dash", line_color="red", annotation_text="National Average: 5.1%")
        ...
        with col2:
            ...
            fig2 = px.scatter(
                ...
                labels={
                    'gp_per_1000': 'GPs per 1,000 people',
                    'obesity_rate': 'Obesity Rate (%)'
                },
                hover_data=['sa1_code', 'seifa_score']
            )
            fig2.add_vline(x=1.0, line_dash="dash", line_color="green", annotation_text="Target: 1.0+ GPs")
            st.plotly_chart(fig2, use_container_width=True)
        
        # Correlation matrix
        st.subheader("🔗 Health Indicator Correlations")
        
        health_cols = ['diabetes_prevalence', 'obesity_rate', 'hypertension_rate', 'mental_health_score', 
                      'gp_per_1000', 'seifa_score', 'median_income', 'education_score']
    ...
    with tab2:
        ...
        with col1:
            # Population by state
            state_pop = filtered_df.groupby('state').agg({
                'population': 'sum',
                'sa1_code': 'count'
            }).reset_index()
        ...
        area_comparison = filtered_df_copy.groupby('area_type').agg({
    ...
    with tab3:
        ...
        with col1:
            ...
            filtered_df_copy['diabetes_risk'] = (filtered_df_copy['diabetes_prevalence'] - filtered_df_copy['diabetes_prevalence'].min()) / (filtered_df_copy['diabetes_prevalence'].max() - filtered_df_copy['diabetes_prevalence'].min())
            filtered_df_copy['obesity_risk'] = (filtered_df_copy['obesity_rate'] - filtered_df_copy['obesity_rate'].min()) / (filtered_df_copy['obesity_rate'].max() - filtered_df_copy['obesity_rate'].min())
            filtered_df_copy['access_risk'] = 1 - ((filtered_df_copy['gp_per_1000'] - filtered_df_copy['gp_per_1000'].min()) / (filtered_df_copy['gp_per_1000'].max() - filtered_df_copy['gp_per_1000'].min()))
        ...
        with col2:
            # Top risk areas
            high_risk = filtered_df_copy.nlargest(10, 'health_risk_score')[['sa1_code', 'state', 'health_risk_score', 'population', 'diabetes_prevalence', 'gp_per_1000']]
    ...
    with tab4:
        ...
        if st.button("📥 Download Filtered Data"):
            ...
            st.download_button(
                label="Download CSV",
                data=csv,
                file_name="ahgd_filtered_data.csv",
                mime="text/csv"
            )
    
    # Footer
    st.markdown("---")
    st.info("📊 **AHGD V3 Platform** - Australian health data analytics with real statistical indicators based on ABS and AIHW data patterns.")
