...
class SA1Coordinates(VersionedSchema):
    ...
    population: Optional[int] = Field(
        ...
        ge=50, 
        le=1200, 
        description="Population count (typical range 200-800)"
    )
    ...
    neighbours: List[str] = Field(
        ...
        description="List of neighbouring SA1 codes"
    )
    ...
    indigenous_region: Optional[str] = Field(
        None,
        description="Indigenous Region code if applicable"
    )
    ...
    def validate_data_integrity(self) -> List[str]:
        ...
        if self.population and self.boundary_data.area_sq_km:
            ...
            elif density < 1 and self.boundary_data.area_sq_km < 10:  # Urban SA1 with very low density
                errors.append(f"Population density unusually low for small area: {density:.1f} per sq km")
...
class SA1GeometryValidation(VersionedSchema):
    ...
    contains_address_points: Optional[int] = Field(
        None,
        ge=0,
        description="Number of address points contained within SA1"
    )
...
class SA1BoundaryRelationship(VersionedSchema):
    ...
    adjacent_sa1s: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="List of adjacent SA1s with shared boundary info"
    )
    ...
    address_count: Optional[int] = Field(
        None,
        ge=0,
        description="Number of addresses within SA1"
    )
    mesh_block_codes: List[str] = Field(
        default_factory=list,
        description="List of Mesh Block codes that comprise this SA1"
    )
    ...
    distance_to_town_centre_km: Optional[float] = Field(
        None,
        ge=0,
        description="Distance to nearest town/city centre in km"
    )
    ...
    def validate_data_integrity(self) -> List[str]:
        ...
        if self.parent_sa2 and not self.sa1_code.startswith(self.parent_sa2):
            errors.append(f"Parent SA2 {self.parent_sa2} inconsistent with SA1 code {self.sa1_code}")
