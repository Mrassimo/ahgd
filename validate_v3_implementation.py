...
def validate_level_2_functionality() -> Dict[str, bool]:
    ...
    try:
        ...
        print_result("Polars DataFrame operations", results['polars_operations'], 
    ...
    try:
        ...
        print_result("DuckDB analytical operations", results['duckdb_operations'],
    ...
    print_result("dbt project structure", dbt_valid, 
    ...
    print_result("Streamlit app structure", streamlit_valid,
                "All required Streamlit components present" if streamlit_valid else "Missing Streamlit components")
...
def validate_level_3_integration() -> Dict[str, bool]:
    ...
    try:
        ...
        print_result("Docker Compose configuration", services_present,
    ...
    try:
        if Path('dbt_project.yml').exists():
            ...
            import subprocess
            result = subprocess.run(['dbt', 'parse'], 
                                  capture_output=True, text=True, cwd='.')
            
            dbt_valid = result.returncode == 0
            results['dbt_compilation'] = dbt_valid
            print_result("dbt model compilation", dbt_valid,
    ...
    try:
        ...
        transformed_data = raw_data.lazy().with_columns([
        ...
        # Validate results
        data_flow_valid = (
            len(analytical_result) == 3 and  # 3 population categories
            processing_time < 2.0 and        # Processing under 2 seconds
            transformed_data.height == 1000  # All records processed
        )
        
        results['data_flow_integration'] = data_flow_valid
        print_result("Data flow integration (Extract→Transform→Load)", data_flow_valid,
...
def validate_level_4_deployment() -> Dict[str, bool]:
    ...
    print_result("Docker image configurations", docker_valid,
    ...
    try:
        ...
        for count in record_counts:
            ...
            result = test_data.lazy().with_columns([
                ...
                (pl.col('health_metric') * pl.col('population') / 100).alias('health_burden'),
                pl.col('health_metric').rank().alias('health_rank'),
                pl.col('population').pct_change().alias('pop_change')
            ]).group_by(
                (pl.col('sa1_code').str.slice(0, 3)).alias('region')
            ).agg([
        ...
        print_result("Performance benchmark (100K records)", performance_valid,
    ...
    try:
        ...
        completeness_check = test_health_data.select([
            ...
            (pl.col('diabetes_prevalence').is_not_null().sum() / pl.len() * 100).alias('diabetes_completeness')
        ])
        ...
        print_result("Data quality standards", quality_valid,
...
""")
    ...
    for level, results in all_results.items():
        ...
        print(f"{status} {level.replace('_', ' ').title()}: {level_passed}/{level_total} ({level_success:.0f}%)")
    
    print(f"\n🏆 OVERALL SUCCESS RATE: {success_rate:.1f}% ({passed_tests}/{total_tests})")
    
    # Production readiness assessment
    if success_rate >= 90:
        print(f"✅ PRODUCTION READY - Implementation meets quality standards")
        return True
    elif success_rate >= 75:
        print(f"⚠️  PRODUCTION PENDING - Some issues need resolution")
        return False
    else:
        print(f"❌ NOT PRODUCTION READY - Major issues require attention")
        return False
